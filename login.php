...
<head>
  ...
  <title>Login | Panacea Pharmaceutical</title>
...
<body class="text-gray-800">
  ...
  <header class="bg-white shadow p-4 flex items-center sticky top-0 z-50">
    ...
    <nav class="space-x-4">
      <a href="index.html" class="text-gray-700 hover:text-primary hover:underline transition">Home</a>
   ...
   <!-- Login -->
  <section class="flex justify-center items-center min-h-screen bg-gray-50 py-10">
  <div class="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
    <h2 class="text-3xl font-playfair mb-6 text-center">Login</h2>
    <form class="space-y-5" method="POST" action="login.php">
      <p class="text-center mt-4 text-red-500 font-light text-sm"><?php if(isset($_GET['error'])){ echo $_GET['error'];} ?></p>
      <div>
        <label for="email" class="block mb-1 font-semibold text-gray-700">Email</label>
        <input type="email" id="email" name="email" placeholder="Your Email" required
               class="w-full p-3 border border-gray-300 rounded focus:outline-none focus:border-blue-600" />
      </div>
      ...
      <button type="submit" name="login" class="w-full text-white font-semibold py-3 rounded bg-customBlue hover:bg-blue-700 transition">
        Log In
      </button>
    </form>
    <p class="text-center mt-4 text-gray-600 font-light text-sm">
